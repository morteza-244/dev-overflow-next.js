...
const GlobalSearchResult = () => {
  ...
  return (
    <div className="absolute top-full w-full mt-3 bg-light-800 dark:bg-dark-400 rounded-md shadow-md py-5">
      <p className="paragraph-semibold text-dark400_light800 px-5">Filters</p>
      <div className="my-4 h-[1px] bg-light-700/100 dark:bg-dark-500/50" />
      <div className="space-y-5">
        <p className="paragraph-semibold text-dark400_light800 px-5">Top Match</p>
        {isLoading ? (
          <ResultLoading />
        ) : (
            ...
            {!data.length ? (
              ...
              data.map((item: any, index) => (
                ...
                <div className="flex flex-col">
                    <p className="body-medium text-dark200-light800 line-clamp-1 body-medium">{item.title}</p>
                    <p className="font-bold text-light400_light500 mt-1 capitalize">{item.type}</p>
                </div>
                </Link>
              ))
            )}
